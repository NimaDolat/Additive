function [DATA, reg, NumPartitionComponent, Scale, BW] = AdaptiveGreedyPolicyObservedReg(boundPara, noise2, numExp, updateIteration, realOpt, InitScale, InitBW, InitDATA, FuncName)
X = InitDATA(:,1:(end-1));
dim = size(X,2);
T = InitDATA(:,end);     
GrandOpt = max(T);
optVec = NaN(1,(numExp+1));
optVec(1) = GrandOpt;
...
%     xRegret = NaN(1, dim);
    ...
    for partIteration = 1:numParts
        ...
        NegUCBfunc = @(test) NegativeUCB(test, Xind, scale, bw, L, alpha, iexp);
        funcUCB = struct('f', NegUCBfunc);
        [~, xatminIdUCB] = Direct(funcUCB, boundsInd);
%             NegIndMean = @(x) NegIndGPmean(x, Xind, y, scale, bw, L);
%             funcMu = struct('f', NegIndMean);
%         [~, xatminIdMu] = Direct(funcMu, boundsInd, [], Xind, scale, bw, alpha);
        xNext(partition{partIteration}) = xatminIdUCB';
