...
parfor iSim = 1:nsim
    ...
    [DATA, reg, Scale, BW] = Full6D(funcEI, funcMu, boundPara, noise2, oneExtExp, updateIteration, realOpt, InitScale, InitBW, InitDATA, FuncName);
    Allscales(iSim,:) = Scale;
    Allbws(iSim,:) = BW;
    AllDATA{iSim}= DATA;
    Allreg(iSim,:) = reg;
end
filename = ['Full6D', FuncName, 'UpFreq', sprintf('%d',updateIteration), '.mat'];
